...
async def test_integration():
    ...
    try:
        ...
        if any(status in negotiation_result for status in ["SUCCESS", "COUNTER-OFFER", "REJECTED", "HUMAN INTERVENTION"]):
...
async def test_performance():
    ...
    try:
        ...
        print(f"✅ Completed {len(tasks)} concurrent requests in {duration:.2f} seconds")
        
        # Check results
        successful = sum(1 for result in results if isinstance(result, str) and "Search Results" in result)
